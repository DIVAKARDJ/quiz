...
class AuthController extends Controller
{
            ...
            if ($user->role == USER_ROLE_USER) {
                ...
                if ($user->active_status == STATUS_SUCCESS) {
                    if ($user->email_verified == STATUS_SUCCESS) {
                       ...
                       $total_point = $this->userPointCheck($user);
                        $data['success'] = true;
        ...
        try {
            ...
            if (isset($user)) {
                ...
                try {
                    ...
                    $createPointWallet = app(CommonService::class)->create_point_wallet($user->id);
                    $mailService = app(MailService::class);
                    $userName = $user->name ;
                    $userEmail = $user->email;
                    $companyName = isset($default['company']) && !empty($default['company']) ? $default['company'] : __('Quiz Test');
   ...
   private function dailyLoginCheck($user){
   ...
   private function userPointCheck($user){
        ...
        try {
            ...
            if ($user) {
               ...
               if (isset($request->referral_code) && !empty($request->referral_code)){
                  app(CommonService::class)->referralBonus($user,$request->referral_code);
               }
                $admob_coin = 0;
                if(!empty(allsetting('admob_coin'))) {
                    $admob_coin = allsetting('admob_coin');
