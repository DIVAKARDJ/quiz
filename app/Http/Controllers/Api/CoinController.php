...
class CoinController extends Controller
{
    ...
    public function coinPrice(){
       $response = app(CoinService::class)->coinPrice();
       return response()->json($response,200);
    }
    public function pointPrice(){
       $response = app(CoinService::class)->pointPrice();
       return response()->json($response,200);
    }
    
    public function payuHashGenerate(Request $request){
       $response = app(CoinService::class)->payuHashGenerate($request);
       return response()->json($response,200);
    }
    public function getPaymentCredential(){
       $response = app(CoinService::class)->getPaymentCredential();
       return response()->json($response,200);
    }
    
    public function coinConversionUsdToRupee(){
       $response = app(CoinService::class)->coinConversionUsdToRupee();
       return response()->json($response,200);
    }
}
