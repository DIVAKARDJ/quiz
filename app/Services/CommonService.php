    ...
    public function userRegistration($request, $mailTemplet, $mail_key)
    {
        ...
        try {
            DB::transaction(function () use ($request, $mailTemplet, $mail_key, $randno) {
                ...
                $this->create_point_wallet($user->id);
                
                if (!empty($request->referral_code)){
                     $this->referralBonus($user,$request->referral_code);
                }
            });
            return [
                'success' => true,
...
//                'message' => __('Something went wrong . Please try again!')
    ...
    public function create_point_wallet($user_id)
    {
        $point = 0;
        if(!empty(allsetting('signup_point'))) {
           $point = allsetting('signup_point');
        }
        $createPointWallet = DB::table('user_points')->insert(['user_id' => $user_id, 'point' => $point,'created_at'=>Carbon::now(),'updated_at'=>Carbon::now()]);
    }
    
    public function referralBonus($user,$referral_code){
       $refferal_user = User::where('referral_code',$referral_code)->first();
       User::where('id','=',$user->id)->update(['affiliate_id'=>$refferal_user->id]);
       if (allsetting('reward_system_as') == 'Coin'){
          $this->referralCoinBonus($user,$refferal_user);
       }elseif (allsetting('reward_system_as') == 'Point'){
          $point = $this->referralPointBonus($user,$refferal_user);
       }
       
    }
    
    private function referralCoinBonus($user,$refferal_user){
       $user_coin_wallet = UserCoin::where('user_id','=',$user->id)->first();
       if ($user_coin_wallet){
          $coin = $user_coin_wallet->coin+allsetting('register_though_affiliate');
          UserCoin::where('user_id','=',$user->id)->update(['coin'=>$coin]);
       }else{
          $this->create_coin_wallet(allsetting('register_though_affiliate'));
       }
       $affiliate_owner_coin_wallet = UserCoin::where('user_id',$refferal_user->id)->first();
   
       if ($affiliate_owner_coin_wallet){
          $coin = $affiliate_owner_coin_wallet->coin+allsetting('affiliate_owner_on_register');
          UserCoin::where('user_id','=',$refferal_user->id)->update(['coin'=>$coin]);
       }else{
          $this->create_coin_wallet(allsetting('affiliate_owner_on_register'));
       }
    }
    
    private function referralPointBonus($user,$refferal_user){
       try {
          $user_point_wallet = DB::table('user_points')->where('user_id','=',$user->id)->first();
          if ($user_point_wallet){
             $point = $user_point_wallet->point+allsetting('register_though_affiliate');
             $update = DB::table('user_points')->where('user_id','=',$user->id)->update(['point'=>$point]);
             if ($update){
                $this->pointDistributionLog('register_though_affiliate',$user->id,allsetting('register_though_affiliate'));
             }
          }else{
             $this->create_point_wallet(allsetting('register_though_affiliate'));
             $this->pointDistributionLog('register_though_affiliate',$user->id,allsetting('register_though_affiliate'));
          }
   
          $affiliate_owner_point_wallet = DB::table('user_points')->where('user_id',$refferal_user->id)->first();
   
          if ($affiliate_owner_point_wallet){
             $point = $affiliate_owner_point_wallet->point+allsetting('affiliate_owner_on_register');
             $refferal_user_update = DB::table('user_points')->where('user_id','=',$refferal_user->id)->update(['point'=>$point]);
             if ($refferal_user_update){
                $this->pointDistributionLog('affiliate_owner_on_register',$refferal_user->id,allsetting('affiliate_owner_on_register'));
             }
          }
          return TRUE;
    ...
    private function pointDistributionLog($slug,$user_id,$point){
       $insert_data = [
          'admin_setting_slug' => $slug,
          'operation_type' => 'Add',
          'user_id' => $user_id,
          'point' => $point,
