                        ...
                        <div class="col-12">
                            ...
                            <table id="category-table" class="table category-table table-bordered  text-center mb-0">
                                ...
                                <tr>
                                    ...
                                    @endif
                                    <td>@if(empty($parentId)) {{ total_question_count(get_sub_category($item->id)) }} @else {{count_question($item->id)}} @endif</td>
                                    <td>{{ $item->coin }}</td>
                                    <td>{{ $item->serial }}</td>
                                    <td>{{ date('d M y', strtotime($item->created_at)) }}</td>
                                    <td><span @if($item->status == 1) class="text-success" @else class="text-danger" @endif>{{ statusType($item->status) }}</span></td>
                                    <td>
                                        <ul class="d-flex justify-content-center activity-menus">
