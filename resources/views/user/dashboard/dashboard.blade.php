                                    ...
                                    <li>
                                        <a @if(check_category_unlock($item->id, $item->coin) == 1)
                                           ...
                                           @else href="{{route('categoryData', encrypt($item->id))}}" @endif >
                                            ...
                                            @endif
{{--                                            {{$item->name}} ({{total_question_count(get_sub_category($item->id)) }})--}}
                                        ...
                                        @foreach($categories as $item)
                                            <div class="col-lg-3 col-sm-6 col-12">
                                                <a @if(check_category_unlock($item->id, $item->coin) == 1)
                                                   onclick='open_modal("{{$item->id}}","{{$item->coin}}","{{$item->name}}");'
                                                    @else href="{{route('categoryData', encrypt($item->id))}}" @endif>
                                                    <div class="category-single @if($in == 1) category-single-2 @elseif($in == 2) category-single-3
                                                        ...
                                                        @if(check_category_unlock($item->id, $item->coin) == 1)
                                                            <div class="lock">
                                                                <span><img src="{{asset('assets/user/images/category/img-10.png')}}" alt=""></span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </a>
